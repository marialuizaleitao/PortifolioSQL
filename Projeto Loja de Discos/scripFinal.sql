...
SELECT bands.name AS 'Nome da Banda' FROM bands;

SELECT * FROM albums WHERE release_year IS NOT NULL
ORDER BY release_year
LIMIT 1;

SELECT DISTINCT bands.name AS 'Nome da Banda' FROM bands
JOIN albums ON bands.id = albums.band_id;

SELECT bands.name AS 'Nome da Banda'
FROM bands
LEFT JOIN albums ON bands.id = albums.band_id
GROUP BY bands.id
HAVING COUNT(albums.band_id) = 0;

SELECT albums.name as 'Nome', albums.release_year as 'Ano de Lançamento', SUM(songs.length) as 'Duração' FROM albums
JOIN songs on albums.id = songs.album_id GROUP BY songs.album_id
...
SELECT AVG(length) as 'Duração Média' FROM songs;

SELECT albums.name AS 'Álbum', albums.release_year AS 'Ano de Lançamento', MAX(songs.length) AS 'Duração' FROM albums
JOIN songs ON albums.id = songs.album_id
GROUP BY songs.album_id;

SELECT bands.name AS 'Bandas', COUNT(songs.id) AS 'Número de Músicas' FROM bands
JOIN albums ON bands.id = albums.band_id
JOIN songs ON albums.id = songs.album_id
